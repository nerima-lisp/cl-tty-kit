...
(defmacro %make-row-dirty-bits (width height)
...
(defmacro %copy-row-dirty-bits (row-dirty-bits)
  `(let ((row-dirty-bits ,row-dirty-bits))
...
(defmacro %screen-touch (screen &optional (start-y 0) (end-y nil end-y-supplied-p)
                                  (start-x 0) (end-x nil end-x-supplied-p))
  `(let ((screen ,screen))
     (let ((start-y ,start-y)
           (end-y ,(if end-y-supplied-p end-y '(screen-height screen)))
           (start-x ,start-x)
           (end-x ,(if end-x-supplied-p end-x '(screen-width screen))))
...
(defmacro %screen-clear-dirty-cells (screen)
  `(let ((screen ,screen))
...
(defmacro %screen-vector (width height &optional (cell nil cell-supplied-p))
  `(let ((width ,width)
         (height ,height)
         (cell ,(if cell-supplied-p cell '(%blank-cell))))
     (declare (type fixnum width height))
...
(defmacro %screen-put-cell-normalized-style (screen x y char style)
