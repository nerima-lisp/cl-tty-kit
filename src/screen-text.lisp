...
the natural way to center a label in a panel carved out with RECT-INSET."
  ...
  (let ((rect-width (rect-width rect))
    ...
    (declare (type fixnum rect-width rect-height))
    (when (and (plusp rect-width) (plusp rect-height))
      (multiple-value-bind (end cells) (%cells-prefix-end text rect-width)
        (let* ((column-offset (ecase align
                                ...
                                (:center (ash (- rect-width cells) -1))))
               (row-offset (ecase vertical
                             (:top 0)
                             (:middle (ash (1- rect-height) -1))
                             (:bottom (1- rect-height))))
               (x (+ (rect-x rect) (if (minusp column-offset) 0 column-offset)))
...
spacer, matching the grid. Useful for snapshots and test assertions."
  ...
  (let* ((width (screen-width screen))
         ...
         (separator-count (if (plusp height) (1- height) 0))
         (result (make-string (+ (* width height) separator-count)))
         (output-index 0)
         (cell-index 0))
    (declare (type fixnum width height separator-count output-index cell-index)
             (type simple-string result))
    (dotimes (y height result)
      (loop repeat width
            do (setf (schar result output-index)
                     (cell-char (aref cells cell-index)))
               (incf output-index)
               (incf cell-index))
      (unless (= y (1- height))
        (setf (schar result output-index) #\Newline)
        (incf output-index)))))
