...
(defmacro %make-screen-diff-plan (screen)
  `(let ((screen ,screen))
...
(defmacro %clear-diff-plan (plan)
  `(let ((plan ,plan))
     (setf (fill-pointer (diff-plan-operations plan)) 0)
     plan))

(defmacro %record-diff-operation (plan start end)
  `(let ((plan ,plan)
         (start ,start)
         (end ,end))
...
(defmacro %render-blank-cell-p (cell)
  `(let ((cell ,cell))
     (and (char= (cell-char cell) #\Space) (null (%cell-style-sequence cell)))))

(defmacro %same-screen-dimensions-p (screen previous)
  `(let ((screen ,screen)
         (previous ,previous))
     (and previous
...
(defmacro %row-blank-suffix-start (cells row-start width)
  `(let ((cells ,cells)
         (row-start ,row-start)
         (width ,width))
...
(defmacro %diff-cursor-length (x y)
  `(let ((x ,x)
         (y ,y))
...
(defmacro %plan-diff-length (screen previous plan &optional changed-since)
...
(defmacro %copy-diff-plan-cells (front back plan)
