...
(defmacro %write-diff-run (cells previous-cells start-index row-end stream)
  `(let ((cells ,cells)
         (previous-cells ,previous-cells)
     ...
     (let ((end-index
            (do ((index start-index (1+ index)))
              ...
              (when (%cell-equal-p (aref cells index)
                                   (aref previous-cells index))
                (return index)))))
       (%write-cell-range cells start-index end-index stream)
       end-index)))

(defmacro %diff-render-length (screen previous max-length)
...
(defmacro %write-diff (screen previous stream)
...
(defmacro %screen-render-length (screen)
  `(let ((screen ,screen))
     (let ((length 10)
       ...
       (declare (type simple-vector cells)
                (type fixnum length width height))
       (do ((row 0 (1+ row))
            (row-start 0 (+ row-start width)))
           ((>= row height))
         (declare (type fixnum row row-start))
         (incf length
               (%cell-range-rendered-length cells row-start (+ row-start width))))
...
(defun %screen-render-length-exceeds-p (screen maximum)
  ...
  (let ((length 10)
        ...
        (height (screen-height screen)))
    (declare (type simple-vector cells)
             (type fixnum length width height maximum))
    (when (> length maximum)
...
(defmacro %minimum-screen-render-length (screen)
...
(defmacro %write-diff-plan (screen plan stream)
  `(let ((screen ,screen)
         (plan ,plan)
         (stream ,stream))
     (let* ((cells (screen-cells screen))
       ...
       (declare (type simple-vector cells)
                ...
                (type fixnum width row row-start row-end))
       (do ((operation-index 0 (+ operation-index 2))
         ...
         (let ((start (aref operations operation-index))
           ...
           ;; Diff-plan operations are emitted in row-major order.
           (loop while (>= start row-end)
                 do (incf row)
                 (setf row-start row-end
                       row-end (+ row-end width)))
           (let ((x (- start row-start)))
             (declare (type fixnum x))
             (%write-ansi-move-cursor (1+ row) (1+ x) stream)
             (if (minusp end)
                 (%write-ansi-clear-line stream)
            ...
            (multiple-value-bind (diff-length too-long-p)
              ...
              (if (or (< diff-length (%minimum-screen-render-length screen))
                  ...
                  (multiple-value-bind (result wrote-output-p)
                    ...
                    (values result wrote-output-p nil))
                  (values (%write-screen screen stream) t t)))))))
(defmacro %render-diff-output (screen previous stream &optional plan changed-since)
  `(let ((screen ,screen)
         (previous ,previous)
         (stream ,stream)
         (plan ,plan)
...
(defmacro %render-frame-diff-output (screen previous cursor previous-cursor stream &optional plan changed-since) `(let ((screen ,screen) (previous ,previous) (cursor ,cursor) (previous-cursor ,previous-cursor) (stream ,stream) (plan ,plan) (changed-since ,changed-since)) (%with-style-sgr-sequence-cache (if stream (%write-frame-diff screen previous cursor previous-cursor stream plan changed-since) (let ((output (make-string-output-stream))) (multiple-value-bind (result diff-output-p full-repaint-p) (%write-frame-diff screen previous cursor previous-cursor output plan changed-since) (declare (ignore result)) (values (get-output-stream-string output) diff-output-p full-repaint-p)))))))

(defmacro %write-frame-diff (screen previous cursor previous-cursor stream &optional plan changed-since) `(let ((screen ,screen) (previous ,previous) (cursor ,cursor) (previous-cursor ,previous-cursor) (stream ,stream) (plan ,plan) (changed-since ,changed-since)) (multiple-value-bind (result diff-output-p full-repaint-p) (%write-preferred-diff screen previous stream plan changed-since) (declare (ignore result)) (if (or diff-output-p (null previous-cursor) (not (%cursor-equal-p cursor previous-cursor))) (values (%write-cursor cursor stream) t full-repaint-p) (values stream nil full-repaint-p)))))
