...
(defmacro with-fresh-input-decoder ((decoder &key collect-bracketed-paste) &body body)
  `(let ((,decoder (make-input-decoder
                    :collect-bracketed-paste ,collect-bracketed-paste)))
     ,@body))
...
(defun %chunk-eof-flags (chunks eof)
  (let ((flags (copy-list eof)))
    (append flags
            (make-list (max 0 (- (length chunks) (length flags)))
                       :initial-element nil))))

(defun %decode-input-streaming-signatures (input splits &key collect-bracketed-paste)
  (with-fresh-input-decoder (decoder :collect-bracketed-paste collect-bracketed-paste)
    (let ((index 0)
          (events '()))
      (dolist (split splits)
...
(defun %assert-streaming-decode-equivalent (input splits
                                            &key collect-bracketed-paste)
  (let ((expected (%decode-input-single-chunk-signatures
                   input
                   :collect-bracketed-paste collect-bracketed-paste))
        (actual (%decode-input-streaming-signatures
                 input splits
                 :collect-bracketed-paste collect-bracketed-paste)))
...
(defun %assert-event-signatures= (events expected message)
  (is (equal (%event-signatures-of events) expected)
      message))
  ...
  (with-fresh-input-decoder (decoder :collect-bracketed-paste collect-bracketed-paste)
    (let ((events '()))
      (loop for chunk in chunks
            ...
            do (setf events
                     (nconc events
                            (decode-input-chunk decoder chunk :eof chunk-eof))))
...
(defun %assert-chunk-case (chunks expected message
                           &key collect-bracketed-paste eof)
  (multiple-value-bind (events)
      (%decode-input-chunks chunks
                            :collect-bracketed-paste collect-bracketed-paste
                            :eof eof)
...
(defun %test-basic-input-cases ()
  (do-test-case-bind (case +basic-input-cases+ (input expected message))
    (%assert-decode-input-case input expected message)))

(defun %test-decoder-smoke-cases ()
  (let ((condition (make-condition 'unsupported-code-point
                                   :code-point #x110000)))
    (is (= #x110000 (unsupported-code-point-code-point condition)))
    (is (search "Unsupported Unicode code point 1114112"
                (format nil "~A" condition))))
  (%assert-invalid-utf8-case
   (lambda () (decode-input #(227 40 130)))
   1
...
(defun %test-streaming-input-cases ()
  (do-test-case-bind (case +streaming-input-cases+
                           (chunks expected message
                            &key collect-bracketed-paste))
    (%assert-chunk-case chunks
                        expected
                        message
                        :collect-bracketed-paste collect-bracketed-paste)))
...
(defun %test-flush-and-error-cases ()
  ...
  (let ((decoder (make-input-decoder)))
    (is (null (decode-input-chunk decoder #(227 129))))
    (%assert-invalid-utf8-case
     (lambda () (flush-input-decoder decoder))
     0
     :truncated-sequence
     "Flushing a partial UTF-8 sequence should signal truncation."))
  (%assert-invalid-utf8-case
   (lambda () (decode-input #(255 97)))
   0
...
(defparameter +input-example-cases+
  `(("examples/key-decoding.lisp"
     ((:character #\a nil)
...
(defun %test-input-examples ()
  (do-test-case-bind (case +input-example-cases+
                           (example-file expected message))
