...
(describe "unify"
  (it "unifies a shared free variable across two terms"
    (flet ((unified (left right)
             (multiple-value-bind (env ok) (cl-prolog-kit:unify left right)
               (and ok (cl-prolog-kit:logic-substitute left env)))))
  ...
  (it "rejects binding one variable to two distinct constants"
    (multiple-value-bind (env ok) (cl-prolog-kit:unify '(?x ?x) '(a b))
      (declare (ignore env))
      (expect (not ok))))
  (it "the occurs check rejects an infinite term"
    (multiple-value-bind (env ok) (cl-prolog-kit:unify '?x '(f ?x))
      (declare (ignore env))
      (expect (not ok))))
  (it "a circular host term fails unification instead of looping"
    ...
    ;; erroring: cl-prolog-kit compares cyclic structure coinductively.
    (let ((term (list 'loop)))
      (setf (rest term) term)
      (multiple-value-bind (env ok) (cl-prolog-kit:unify term '(loop))
        (declare (ignore env))
        (expect (not ok))))))
