...
(describe "prolog builtin predicates"
  ...
  ;; N arguments" for a wrong arity), cl-prolog-kit's builtins are registered per
  ...
  (it "a wrong-arity findall call signals prolog-existence-error"
    (let ((db (%genealogy-db)))
      (expect (lambda () (cl-prolog-kit:query-prolog db (list 'cl-prolog-kit:findall '?x '(parent ?x ?y))))
              :to-throw 'cl-prolog-kit:prolog-existence-error)))
  (it "a wrong-arity = call signals prolog-existence-error"
    (expect (lambda () (cl-prolog-kit:query-prolog (%genealogy-db) '(= ?x)))
            :to-throw 'cl-prolog-kit:prolog-existence-error))
  (it "a wrong-arity true call signals prolog-existence-error"
    (expect (lambda () (cl-prolog-kit:query-prolog (%genealogy-db) (list 'cl-prolog-kit:true 'extra)))
            :to-throw 'cl-prolog-kit:prolog-existence-error))
  ...
  (it "call/n appends its extra argument to the inner goal"
    (expect (lambda ()
              (cl-prolog-kit:query-prolog (%genealogy-db)
                                      (list 'cl-prolog-kit:call '(parent abraham isaac) 'extra)))
            :to-throw 'cl-prolog-kit:prolog-existence-error)))
