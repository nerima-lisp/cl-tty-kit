...
;;; --------------------------------------------------------------------------
(describe
  "screen mutation sequence"
  (it
    "threads a screen through resize, put-cell, clear, write-string, and fill-rect"
...
(describe
  "screen initial-cell sharing"
  (it
    "shares the same cell object across every position filled from a character :initial-cell"
    (let ((filled (make-screen 2 1 :initial-cell #\X)))
      (expect-cell (filled 0 0) #\X)
      (expect-cell (filled 1 0) #\X)
      (expect
        (aref (cl-tty-kit::screen-cells filled) 0)
        :to-be
        (aref (cl-tty-kit::screen-cells filled) 1))))
  (it
    "reuses the exact cell object supplied as :initial-cell"
  ...
  (it
    "shares the supplied cell object across every position, with no setf accessors defined"
  ...
  (it
    "reuses the exact cell object supplied to screen-clear's :cell across every position"
  ...
  (it
    "does not advance generation for idempotent writes"
    (let ((screen (make-screen 4 1)))
      (let ((generation (cl-tty-kit::screen-generation screen)))
        ...
        (expect
          (setf (screen-cell screen 0 0) #\Space)
          :to-be
          screen)
        (expect (cl-tty-kit::screen-generation screen) :to-be generation))
      ...
      (let ((generation (cl-tty-kit::screen-generation screen)))
        (screen-put-cell screen 0 0 #\X)
        (expect (cl-tty-kit::screen-generation screen) :to-be generation))
      (screen-write-string screen 0 0 "HUD!" :style (list :bold))
      (let ((generation (cl-tty-kit::screen-generation screen)))
        (screen-write-string screen 0 0 "HUD!" :style (list :bold))
        (expect (cl-tty-kit::screen-generation screen) :to-be generation))
      (screen-fill screen #\X :style (list :underline))
      (let ((generation (cl-tty-kit::screen-generation screen)))
        (screen-fill screen #\X :style (list :underline))
        (expect (cl-tty-kit::screen-generation screen) :to-be generation)))))
...
(describe
  ...
  (it
    "writes nothing when :start equals :end"
  ...
  (it
    "signals an error and leaves the screen unchanged for invalid :start/:end combinations"
    (let ((unchanged (make-screen 3 1 :initial-cell #\.)))
      (expect
        (lambda ()
...
(describe
  "screen-write-string style copy-on-write"
  (it
    "copies :style so later caller-side mutation does not affect already-written cells"
...
(describe
  "screen wide-glyph placement"
  (it
    "reserves a trailing blank cell after a wide glyph, followed by a narrow char"
  ...
  (it
    "gives a zero-width combining character its own cell"
  ...
  (it
    "reserves a trailing blank cell when a narrow char precedes a wide glyph"
    (let ((ideograph (code-char #x65E5)))
  ...
  (it
    "signals before changing any cell when a combining sequence overflows"
    (let* ((combining (code-char #x0301))
           (text (format nil "e~Ca" combining))
           (screen (make-screen 2 1 :initial-cell #\.)))
      (bounds-error-is (condition 2 0 2 1) (screen-write-string screen 0 0 text))
      (expect-cell (screen 0 0) #\.)
      (expect-cell (screen 1 0) #\.)))
  (it
    "applies style to both the wide glyph cell and its blank companion"
    (let ((ideograph (code-char #x65E5)))
  ...
  (it
    "signals screen-index-out-of-bounds when a wide glyph would overflow the last column"
    (let ((ideograph (code-char #x65E5)))
      (bounds-error-is
        (condition 1 0 1 1)
...
(describe
  "cell copy-on-write"
  (it
    "produces a distinct cell object with equal char/style, with no setf accessors defined"
  ...
  (it
    "keeps the copy's style independent of the source cell after mutating the caller-owned list"
    (let* ((style (list :bold))
      ...
      (expect (cell-style copy) :to-equal '(:bold))))
  (it
    "keeps a cell's own style independent of the caller-owned list after construction"
    (let* ((style (list :underline))
           (cell (make-cell :char #\M :style style)))
      (setf (car style) :reverse)
      (expect (cell-style cell) :to-equal '(:underline))))
  (it
    "deep-copies nested style parameter lists so mutating them after copy does not leak"
    (let* ((style (list :bold (list :fg 33) (list :bg 99)))
           (cell (make-cell :char #\K :style style))
           (copy (copy-cell cell)))
      (setf (second (second style)) 44
            (second (third style)) 88)
      (expect (cell-style cell) :to-equal '(:bold (:fg 33) (:bg 99)))
      (expect (cell-style copy) :to-equal '(:bold (:fg 33) (:bg 99)))))
  (it
    "does not expose the internal style list for in-place mutation by callers"
...
(describe
  "render-diff style order independence"
  (it
    "reports no diff when only the order of independent style attributes differs"
  ...
  (it
    "reports no diff when only the order of fg/bg parameters differs"
    (let ((cells-a (make-screen 1 1))
          (cells-b (make-screen 1 1)))
...
(describe
  "cell style normalization"
  (it
    "keeps a style list already in canonical order without duplicate channels"
    (let ((cell (make-cell :char #\C :style (quote (:bold (:fg 196) (:bg 17))))))
      (expect (cell-style cell) :to-equal (quote (:bold (:fg 196) (:bg 17))))))
  (it
    "keeps only the last occurrence of each repeated color channel"
    (let ((cell (make-cell :char #\C :style '((:fg 12) (:fg 196) (:bg 1) (:bg 17)))))
      (expect (cell-style cell) :to-equal '((:fg 196) (:bg 17)))))
  (it
    "preserves 24-bit rgb triples for fg/bg channels"
    (let ((cell (make-cell :char #\C :style '(:underline (:fg 1 2 3) (:bg 4 5 6)))))
      (expect (cell-style cell) :to-equal '(:underline (:fg 1 2 3) (:bg 4 5 6)))))
  (it
    "drops malformed fg/bg channel values, keeping the last valid occurrence"
    (let ((cell (make-cell :char #\C :style '(:bold (:fg 256) (:bg 1 2) (:fg 7)))))
      (expect (cell-style cell) :to-equal '(:bold (:fg 7)))))
  (it
    "drops a malformed dotted channel entry, keeping other valid attributes"
    (let ((cell (make-cell :char #\C :style '((:fg . 1) :bold))))
      (expect (cell-style cell) :to-equal '(:bold))))
  (it
    "make-style returns nil for a wholly malformed style list"
    (expect (make-style '(:fg . 1)) :to-be-null))
  (it
    "defaults to a space character with no style"
    (let ((cell (make-cell)))
      (expect (cell-char cell) :to-be #\Space)
      (expect (cell-style cell) :to-be-null)))
  (it
    "signals an error for a non-character :char"
    (expect
      (lambda ()
        (make-cell :char "C"))
      :to-throw
      'error))
  (it
    "signals an error when copying a non-cell"
    (expect
      (lambda ()
      ...
      'error))
  (it
    "wraps a bare channel keyword/value pair missing its outer list in canonical form"
    (let ((cell (make-cell :char #\C :style '(:fg 33))))
      (expect (cell-style cell) :to-equal '((:fg 33)))))
  (it
    "keeps a nil style as nil"
    (let ((cell (make-cell :char #\C :style nil)))
      (expect (cell-style cell) :to-be-null)))
  (it
    "make-style keeps the last of repeated channel builders"
    (expect
      (make-style :bold (style-fg 12) (style-fg 196) (style-bg 1 2 3))
      :to-equal
      '(:bold (:fg 196) (:bg 1 2 3))))
  (it
    "accepts a style built via make-style/style-fg/style-bg helpers"
    (let ((cell
          (make-cell
            :char
            #\C
  ...
  (it
    "style-fg signals an error for an out-of-range 256-color index"
    (expect
      (lambda ()
        (style-fg 256))
      :to-throw
      'error))
  (it
    "style-fg signals an error for an out-of-range rgb component"
    (expect
      (lambda ()
        (style-fg 1 2 999))
      :to-throw
      'error))
  (it
    "style-bg signals an error for a malformed argument count"
    (expect
      (lambda ()
        (style-bg 1 2))
      :to-throw
