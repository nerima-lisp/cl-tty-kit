...
(defmacro with-step-timeout ((label) &body body)
  `(handler-case
       (sb-ext:with-timeout *verify-timeout-seconds*
         ,@body)
     (sb-ext:timeout ()
...
(defun run-source-registry-smoke ()
  (let* ((scripts-dir (uiop:pathname-directory-pathname *load-truename*))
    ...
    (finish-output)
    (with-step-timeout ("source-registry smoke")
      (uiop:run-program (list "sbcl" "--script" (namestring smoke-script))
